...
![](READEMEPictures/Screen%20Shot%202022-07-20%20at%2011.20.18%20AM.png)
...
#### 1. How to GET all orders from the collection *orders* in the keyspace *keyspacename1* for the username *Andy1*:

Notice that ASTRA_DB_ID, ASTRA_DB_REGION, ASTRA_DB_TOKEN need to be replaced by their correct values depending on the database itself.
Also the collection “orders” has to have been created in your database.

```bash
$ curl -X 'GET'   
    'https://ASTRA_DB_ID-ASTRA_DB_REGION.apps.astra.datastax.com/api/rest/v2/namespaces/keyspacename1/collections/orders?where=\{"userName":\{"$eq":"Andy1"\}\}&page-size=20'   
...
#### 2. How to GET the user info for the username *Andy1* in the collection *userInfo* in the keyspace *keyspacename1*:
Notice that the collection *userInfo* has to have been created in your database.

```bash
$ curl -X 'GET'   
    'https://ASTRA_DB_ID-ASTRA_DB_REGION.apps.astra.datastax.com/api/rest/v2/namespaces/keyspacename1/collections/userInfo?where=\{"userName":\{"$eq":"Andy1"\}\}'   
    -H 'accept: application/json'   
    -H 'X-Cassandra-Token: ASTRA_DB_TOKEN'
```
...
#### 3. How to POST a new user info with username *testUsername* and password *testPassword* in the *userInfo* collection in the keyspace *keyspacename1*:

```bash
$ curl -X 'POST'   
    'https://ASTRA_DB_ID-ASTRA_DB_REGION.apps.astra.datastax.com/api/rest/v2/namespaces/keyspacename1/collections/userInfo'   
    -H 'accept: application/json'   
    -H 'X-Cassandra-Token: ASTRA_DB_TOKEN' 
